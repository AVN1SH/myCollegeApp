...
const LeftNavBar = () => {
  ...
  return userData && (
    <div className="flex fixed bg-white min-w-[66px] lg:min-w-[280px] h-[calc(100%-64px)] shadow-[0_0_7px_#7e7e7e] mt-16">
      <div className={classNames(userData ? "pb-[70px]" : '', "flex flex-col items-start space-y-7 w-full h-full p-3 pt-3 overflow-y-auto custom-scroll-bar transition-[1s]")}>
          ...
          if((userData?.role === 'faculty') || admissionStats ? (item.list === 'both' || item.list === '2') : (item.list === '1' || item.list === 'both')){
            return <div className="w-full whitespace-nowrap transition-[1s]" key={item.name}>
                ...
                className={ classNames(
                  path.includes(item.href) ? "bg-orange-100 text-orange-600" : "hover:text-slate-900 hover:bg-gray-100 text-slate-500" , "flex items-center gap-3 w-full font-semibold p-2 rounded-md  hover:pl-3 duration-75 transition-[1s]"
                )}
        ...
        {userData && <div className="fixed flex items-center gap-4 p-3 lg:pl-4 bg-white border-t-[1px] border-slate-300 border-solid bottom-0 left-0 lg:w-[277.5px] h-[60px] hover:cursor-pointer hover:bg-orange-500 hover:text-white duration-300 w-[66px]">
